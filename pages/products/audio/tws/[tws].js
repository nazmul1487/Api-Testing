...
const TWSData = require('@/JsonData/DB.json');
import Error from 'next/error';
import ProductDetailPage from '@/components/product/ProductDetailPage';
import Link from 'next/link';

export default function TwsAudio( data ) {
...
// }
TwsAudio.getInitialProps = async ({ query: { tws } }) => {
 // const { data } = await ArticleAPI.get(pid);
  const data = await TWSData.TWS.filter(item=> item.slug == tws)
